...
func (r *awsRoleEntry) validate() error {
	...
	if r.ConsoleLogin && r.ConsoleDuration == "" {
		errors = multierror.Append(errors, fmt.Errorf("Duration must be set of AWS Console Login is required, minimum duration 900s (15min) to 129,600s (36hours)"))
	}
